...
- **Capture at the speed of thought — actively and passively.** The hardest part of any second brain is the friction between having a thought and storing it usefully. The system minimizes that friction two ways. **Actively** — dictation, brain-dumps, reaction-capture — preserving what's captured *in the owner's exact words* (keywords are sacred for retrieval; the system never paraphrases the owner's original thinking). And **passively** — scheduled jobs that pull your own trail in without you lifting a finger: your X likes and bookmarks, your reading, your highlights — auto-filed, auto-connected, and turned into to-dos and follow-ups. Capture happens even when you aren't deliberately capturing.
...
- **`/setup` is the front door** — it interviews the owner and fills every `__FILL_FROM_USER__` marker, including the Owner Intent section above (purpose, gaps, goals, privacy register, toolchain, and which credentials to wire into `.env`). It also **gets the environment runnable** — verifying Python and Git, creating a virtual environment, and installing the brain's dependencies — so the brain can install and run its own tooling out of the box, surfacing a manual step only when something can't be set up automatically.
